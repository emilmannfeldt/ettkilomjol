...
const log = [];
const filename = 'tasteline/Newtasteline-2400-2018-07-24';
...
function createRecipes() {
  fs.readFile(`C:/react/${filename}.json`, 'utf8', (err, data) => {
    if (err) {
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      for (let i = 0; i < existingRecipes.length; i++) {
        if (existingRecipes[i].source === recipe.source) {
          ...
          recipesRef.orderByChild('source').equalTo(existingRecipes[i].source).once('value', (snapshot) => {
            snapshot.forEach((child) => {
    ...
    fs.writeFile(`C:/react/${filename}-LOG.json`, JSON.stringify(log), (err) => {
      if (err) {
        return console.log(err);
      }
      log.push('logfile saved!');
    });
    fs.writeFile(`C:/react/recipesbackup/${filename}.json`, JSON.stringify(final), (err) => {
      if (err) {
        return console.log(err);
      }
